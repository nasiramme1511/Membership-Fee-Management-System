...
exports.verify = async (transactionId, amount, receiverAccount, receiverName, receiptUrl) => {
  try {
    const response = await verifyet.verifyTransaction('ebirr', transactionId);

    if (response.statusCode >= 500) {
      return { verified: false, serverError: true, error: 'E-Birr verification server is temporarily unavailable. Please try again later or upload a receipt file instead.' };
    }
    ...
    if (verification.processingStatus === 'queued' || verification.processingStatus === 'running') {
      return { verified: false, serverError: true, error: 'E-Birr verification is taking longer than expected. The payment has been saved for manual review.' };
    }
    ...
    if (msg === 'Timeout' || msg.startsWith('Network error') || msg === 'VERIFY_ET_API_KEY is not configured') {
      return { verified: false, serverError: true, error: `E-Birr verification unavailable: ${msg}` };
    }
    return { verified: false, error: `E-Birr verification failed: ${msg}` };
  }
};
