...
export default function About({ content, aboutImg }: AboutProps) {
  ...
  const aboutImages = ['/about1.png', '/about2.png'];
  ...
  return (
    <section id="about" className="py-28 lg:py-36 bg-gray-50 dark:bg-slate-950 relative overflow-hidden" style={{ perspective: '1200px' }}>
        ...
        <div className="grid lg:grid-cols-12 gap-16 lg:gap-20 items-center">
            ...
            <div className="relative h-[300px] sm:h-[520px]">
              ...
              {aboutImages.map((src, index) => (
                <motion.img
                  key={index}
                  src={src}
                  alt={`${t('landing.about_img_alt')} ${index + 1}`}
                  className="absolute inset-0 w-full h-full object-cover rounded-2xl shadow-2xl"
                  animate={{ opacity: index === currentImageIndex ? 1 : 0 }}
                  transition={{ duration: 1.2, ease: "easeInOut" }}
                  onError={(e) => { (e.target as HTMLImageElement).src = aboutImg || '/photos/gate.jpg'; }}
                />
              ))}

              <div className="absolute -bottom-6 right-6 bg-[#0B5D3B] text-white px-8 py-6 rounded-2xl shadow-2xl border border-[#0B5D3B]/50 z-10">
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-6 pt-4">
              <motion.div
                whileHover={{ scale: 1.03, z: 20, rotateY: -3 }}
                className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-gray-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow"
                style={{ transformStyle: 'preserve-3d' }}
              >
                <div className="flex items-center gap-3 mb-2" style={{ transform: 'translateZ(15px)' }}>
                ...
                <p className="text-gray-500 dark:text-gray-400 text-xs leading-relaxed" style={{ transform: 'translateZ(10px)' }}>
